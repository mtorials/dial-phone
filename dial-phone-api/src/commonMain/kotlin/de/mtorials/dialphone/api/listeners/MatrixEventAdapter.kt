...
abstract class MatrixEventAdapter<T : MatrixEvent>(
    ...
    override fun onNewRoomEvent(event: MatrixEvent, roomId: String, phone: DialPhoneApi) {
        if (type.isInstance(event)) onMatrixEvent(event as T,
            de.mtorials.dialphone.api.entities.entityfutures.RoomFutureImpl(roomId, phone)
        )
    }
    @Suppress
     ...
     */
    abstract fun onMatrixEvent(event: T, roomFuture: de.mtorials.dialphone.api.entities.entityfutures.RoomFutureImpl)
}