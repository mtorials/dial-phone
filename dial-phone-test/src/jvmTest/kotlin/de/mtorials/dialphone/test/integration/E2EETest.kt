...
class E2EETest {
    ...
    @BeforeTest
    fun `register and login as user`() : Unit = runBlocking {
    ...
    fun `send and receive`() : Unit = runBlocking {
        var gotFirstMsg = false
        delay(2000)
        user2.addListeners(ListenerAdapter {
            onRoomMessageReceived {
                gotFirstMsg = (it.room.encrypted && it.message.content.body == TEXT)
            }
        })
        ...
        user2.addListeners(ListenerAdapter {
            onRoomMessageReceived {
                if (it.room.encrypted && gotFirstMsg) assertEquals(it.message.content.body, TEXT_ANSWER)
            }
        })
